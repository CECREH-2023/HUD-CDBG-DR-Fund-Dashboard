(() => {
  ...
  async function renderQuickReport(report) {
    ...
    const kpiGap = 0.010;
    const kpiWidth = (0.96 - ((kpis.length - 1) * kpiGap)) / kpis.length;
    ...
    shapes.push(
      { type: "rect", xref: "paper", yref: "paper", x0: 0.515, x1: 0.98, y0: 0.40, y1: 0.765, line: { color: "#d7e0ea", width: 1 }, fillcolor: "#f8fafc", layer: "below" },
    );
  ...
  function downloadQuickReportCsv() {
    ...
    add("This report-data export contains aggregate financial and geographic results only, not raw financial rows.");
  ...
  function renderKpis(panel, result) {
    ...
    for (const rowIndex of frame) {
      const row = ROWS[rowIndex];
      grants.add(row[C.grantCode]);
      if (row[C.project] >= 0) projects.add(row[C.project]);
      activities.add(row[C.activityCode]);
    ...
    const cards = [
      ...
      ["Unique projects", formatInteger.format(projects.size), "Distinct project titles"],
  ...
  function downloadAggregateCsv(panel) {
    ...
    try {
      ...
      const notes = [
        ...
        "This export contains aggregate financial and geographic results only, not raw financial rows.",
      ];
  ...
  async function initialize() {
    try {
      ...
      setLoading("Creating the Explore & Compare dashboard and Quick Report…", 74);
      renderHeader();
      setupModeTabs();
      await setupQuickReport();
      app.hidden = false;

      setLoading("Preparing the default Explore & Compare view…", 90);
      await switchMode("explore");
